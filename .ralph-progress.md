...
## Story 4.2: Jira Webhook Receiver & Validation Pipeline — PLANNING

### Files to Create/Modify (in order)
1. `src/lib/schemas/jira-webhook-schema.ts` — Zod schemas: jiraWebhookEventSchema, jiraIssuePayloadSchema
2. `src/app/api/webhooks/jira/route.ts` — Route Handler with full pipeline
3. `src/lib/schemas/jira-webhook-schema.test.ts` — Unit tests for schemas

### Pipeline Sequence (exact)
HMAC-SHA256 validate → Event ID dedup → Zod parse → Supabase upsert → revalidateTag('issues') → Realtime broadcast → 200 OK

### Key Patterns
- Use `src/lib/supabase/admin.ts` (service role, bypasses RLS) for all DB ops
- Correlation ID: `crypto.randomUUID()` generated at start, flows through entire pipeline
- Structured logging: `{ message, correlationId, source: "webhook:jira", data }`
- Dead letter: write raw payload + error to dead_letter_events on any pipeline failure
- Return `NextResponse.json({ data, error }, { status })` per code-style rules
- HMAC: `x-hub-signature-256` header with `sha256=...` prefix (Atlassian uses this format)
- Realtime broadcast: `supabaseAdmin.channel('dashboard:developer').send(...)` etc.
- revalidateTag: import from 'next/cache'

### Dependencies/Imports Needed
- crypto (Node built-in, for HMAC)
- next/cache (for revalidateTag)
