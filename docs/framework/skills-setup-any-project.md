...
- a mirrored skill catalog for Claude, OpenCode, Codex, Cursor, Gemini CLI, and Antigravity
...
Install baseline skills into all roots so each runtime can operate independently.
...
If the repo already tracks `.claude/commands`, `.opencode/command`, `.gemini/commands`, `.cursor/commands`, or `.agent/workflows`, keep those explicitly unignored.
